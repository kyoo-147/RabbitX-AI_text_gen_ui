...
def give_VRAM_priority(actor):
    ...
    if actor == 'SD':
        unload_model()
        print("Yêu cầu Auto1111 tải lại điểm kiểm tra cuối cùng được sử dụng...")
        response = requests.post(url=f'{params["address"]}/sdapi/v1/reload-checkpoint', json='')
        response.raise_for_status()

    elif actor == 'LLM':
        print("Yêu cầu Auto1111 giải phóng VRAM...")
        response = requests.post(url=f'{params["address"]}/sdapi/v1/unload-checkpoint', json='')
        response.raise_for_status()
        reload_model()

    elif actor == 'set':
        print("Đã kích hoạt quản lý VRAM -- yêu cầu Auto1111 loại bỏ VRAM...")
        response = requests.post(url=f'{params["address"]}/sdapi/v1/unload-checkpoint', json='')
        response.raise_for_status()

    elif actor == 'reset':
        print("Quản lý VRAM đã bị vô hiệu hóa - yêu cầu Auto1111 tải lại điểm kiểm tra")
        response = requests.post(url=f'{params["address"]}/sdapi/v1/reload-checkpoint', json='')
        response.raise_for_status()

    else:
        raise RuntimeError(f'Quản lý VRAM: "{actor}" không phải là một trạng thái đã biết!')
...
def get_SD_pictures(description, character):
    ...
    print(f'Nhắc trình tạo hình ảnh thông qua API trên {params["address"]}...')
...
def output_modifier(string, state):
    ...
    if string == '':
        string = 'không có mô tả khả thi nào trong phản hồi, hãy thử tạo lại'
        return string

    text = ""
    if (params['mode'] < 2):
        toggle_generation(False)
        text = f'*Gửi một bức ảnh miêu tả: “{string}”*'
    else:
        text = string
...
def toggle_generation(*args):
    ...
    shared.processing_message = "*Đang gửi ảnh...*" if picture_response else "*Đang nhập...*"
...
def SD_api_address_update(address):
    ...
    msg = "✔️ SD API được tìm thấy trên:"
    ...
    try:
        ...
        msg = "❌ Không bật điểm cuối API SD:"
...
def ui():
    ...
    with gr.Accordion("Tham sô", open=True, elem_classes="SDAP"):
        with gr.Row():
            address = gr.Textbox(placeholder=params['address'], value=params['address'], label='Địa chỉ Auto1111\'s WebUI ')
            modes_list = ["Manual", "Immersive/Interactive", "Picturebook/Adventure"]
            mode = gr.Dropdown(modes_list, value=modes_list[params['mode']], label="Phương thức hoạt động", type="index")
            with gr.Column(scale=1, min_width=300):
                manage_VRAM = gr.Checkbox(value=params['manage_VRAM'], label='Quản lý VRAM')
                save_img = gr.Checkbox(value=params['save_img'], label='Giữ hình ảnh gốc và sử dụng chúng trong trò chuyện')

            force_pic = gr.Button("Buộc phản hồi hình ảnh")
            suppr_pic = gr.Button("Ngăn chặn phản hồi hình ảnh")
        with gr.Row():
            checkpoint = gr.Dropdown(params['checkpoint_list'], value=params['sd_checkpoint'], label="Điểm kiểm tra", type="value")
            update_checkpoints = gr.Button("Lấy danh sách các điểm kiểm tra")

        with gr.Accordion("Thông số thế hệ", open=False):
            prompt_prefix = gr.Textbox(placeholder=params['prompt_prefix'], value=params['prompt_prefix'], label='Tiền tố nhắc nhở (dùng tốt nhất để mô tả hình dáng của nhân vật)')
            textgen_prefix = gr.Textbox(placeholder=params['textgen_prefix'], value=params['textgen_prefix'], label='tiền tố textgen (gõ [chủ đề] nơi nên đặt chủ đề)')
            negative_prompt = gr.Textbox(placeholder=params['negative_prompt'], value=params['negative_prompt'], label='Lời nhắc tiêu cực')
            with gr.Row():
                with gr.Column():
                    width = gr.Slider(64, 2048, value=params['width'], step=64, label='Độ rộng')
                    height = gr.Slider(64, 2048, value=params['height'], step=64, label='Độ cao')
                with gr.Column(variant="compact", elem_id="sampler_col"):
                    with gr.Row(elem_id="sampler_row"):
                        sampler_name = gr.Dropdown(value=params['sampler_name'], allow_custom_value=True, label='Phương pháp lấy mẫu', elem_id="sampler_box")
                        create_refresh_button(sampler_name, lambda: None, lambda: {'choices': get_samplers()}, 'refresh-button')
                    steps = gr.Slider(1, 150, value=params['steps'], step=1, label="Các bước lấy mẫu", elem_id="steps_box")
            with gr.Row():
                seed = gr.Number(label="Hạt giống", value=params['seed'], elem_id="seed_box")
                cfg_scale = gr.Number(label="Cân CFG", value=params['cfg_scale'], elem_id="cfg_box")
                with gr.Column() as hr_options:
                    restore_faces = gr.Checkbox(value=params['restore_faces'], label='Khôi phục khuôn mặt')
            ...
            with gr.Row(visible=params['enable_hr'], elem_classes="hires_opts") as hr_options:
                hr_scale = gr.Slider(1, 4, value=params['hr_scale'], step=0.1, label='Cao cấp bởi')
                denoising_strength = gr.Slider(0, 1, value=params['denoising_strength'], step=0.01, label='Sức mạnh khử nhiễu')
                hr_upscaler = gr.Textbox(placeholder=params['hr_upscaler'], value=params['hr_upscaler'], label='Nâng cấp')
