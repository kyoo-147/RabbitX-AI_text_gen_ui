...
def feed_data_into_collector(corpus, chunk_len, chunk_sep):
    ...
    cumulative += "Phá vỡ tập dữ liệu đầu vào...\n\n"
    yield cumulative
    if chunk_sep:
    ...
    cumulative += f"{len(data_chunks)} các khối đã được tìm thấy.\in\Thêm các khối vào cơ sở dữ liệu...\n\n"
    yield cumulative
    add_chunks_to_collector(data_chunks, collector)
    cumulative += "Hoàn thành."
    yield cumulative


def feed_file_into_collector(file, chunk_len, chunk_sep):
    yield 'Đọc dữ liệu đầu vào...\n\n'
...
def feed_url_into_collector(urls, chunk_len, chunk_sep, strong_cleanup, threads):
    ...
    cumulative += f'Đang tải {len(urls)} URLs với {threads} luồng...\n\n'
    yield cumulative
    for update, contents in download_urls(urls, threads=threads):
        yield cumulative + update

    cumulative += 'Xử lý các nguồn HTML...'
    yield cumulative
...
def apply_settings(chunk_count, chunk_count_initial, time_weight):
    ...
    yield f"Các cài đặt sau hiện đang hoạt động: {str(settings_to_display)}"
...
def custom_generate_chat_prompt(user_input, state, **kwargs):
    ...
    if state['mode'] == 'instruct':
        ...
        additional_context = '\nCâu trả lời của bạn phải dựa trên ngữ cảnh bên dưới:\n\n' + '\n'.join(results)
        ...
        if len(history['internal']) > params['chunk_count'] and user_input != '':
            ...
            try:
                ...
                logger.warning(f'Thêm bối cảnh mới sau đây:\n{additional_context}')
                ...
                logger.error("Không thể truy vấn cơ sở dữ liệu, hãy tiếp tục...")
...
def ui():
    ...
    with gr.Row():
        with gr.Column(min_width=600):
            with gr.Tab("Nhập vào văn bản"):
                data_input = gr.Textbox(lines=20, label='Dữ liệu đầu vào')
                update_data = gr.Button('Tải dữ liệu')

            with gr.Tab("Đầu vào URL"):
                url_input = gr.Textbox(lines=10, label='Đầu vào URLs', info='Nhập một hoặc nhiều URL được phân tách bằng ký tự dòng mới.')
                strong_cleanup = gr.Checkbox(value=params['strong_cleanup'], label='Dọn dẹp mạnh mẽ', info='Chỉ giữ các phần tử html trông giống như văn bản dạng dài.')
                threads = gr.Number(value=params['threads'], label='Luồng', info='Số lượng chủ đề sẽ sử dụng khi tải xuống URL.', precision=0)
                update_url = gr.Button('Tải dữ liệu')

            with gr.Tab("Tệp đầu vào"):
                file_input = gr.File(label='Tập tin đầu vào', type='binary')
                update_file = gr.Button('Tải dữ liệu')

            with gr.Tab("Cài đặt khởi tạo"):
                chunk_count = gr.Number(value=params['chunk_count'], label='Số lượng đoạn', info='Số đoạn khớp gần nhất cần đưa vào lời nhắc.')
                gr.Markdown('Trọng số thời gian (tùy chọn, được sử dụng để làm cho các phần được thêm gần đây có nhiều khả năng xuất hiện hơn)')
                time_weight = gr.Slider(0, 1, value=params['time_weight'], label='Trọng lượng thời gian', info='Xác định cường độ của trọng số thời gian. 0 = không có trọng số thời gian.')
                chunk_count_initial = gr.Number(value=params['chunk_count_initial'], label='Số đoạn ban đầu', info='Số lượng đoạn khớp gần nhất được truy xuất để sắp xếp lại trọng số thời gian trong chế độ trò chuyện. Đây phải là >= số lượng đoạn. -1 = Tất cả các khối được lấy ra. Chỉ được sử dụng nếu time_weight > 0.')

                update_settings = gr.Button('Áp dụng các thay đổi')

            chunk_len = gr.Number(value=params['chunk_length'], label='Chiều dài đoạn', info='Trong ký tự, không phải mã thông báo. Giá trị này được sử dụng khi bạn nhấp vào "Tải dữ liệu".')
            chunk_sep = gr.Textbox(value=params['chunk_separator'], label='Dấu phân cách', info='Được sử dụng để chia các phần theo cách thủ công. Các đoạn được chia thủ công dài hơn độ dài đoạn sẽ được chia lại. Giá trị này được sử dụng khi bạn nhấp vào "Tải dữ liệu".')
        with gr.Column():
