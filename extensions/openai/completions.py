...
def convert_history(history):
    '''
    Lịch sử trò chuyện trong chương trình này có định dạng [tin nhắn, trả lời].
    Chức năng này chuyển đổi lịch sử OpenAI sang định dạng đó.
    '''
    chat_dialogue = []
    current_message = ""
    current_reply = ""
        ...
        if "image_url" in entry:
            ...
            if "base64" in image_url:
                ...
                try:
                    ...
                    raise 'Không thể tải hình ảnh từ URL!'
...
def chat_completions_common(body: dict, is_legacy: bool = False, stream=False) -> dict:
    if body.get('functions', []):
        raise InvalidRequestError(message="Hàm không được hỗ trợ.", param='functions')

    if body.get('function_call', ''):
        raise InvalidRequestError(message="function_call không được hỗ trợ.", param='function_call')

    if 'messages' not in body:
        raise InvalidRequestError(message="tin nhắn là bắt buộc", param='messages')

    messages = body['messages']
    for m in messages:
        if 'role' not in m:
            raise InvalidRequestError(message="tin nhắn: thiếu vai trò", param='messages')
        elif m['role'] == 'function':
            raise InvalidRequestError(message="vai trò: chức năng không được hỗ trợ.", param='messages')

        if 'content' not in m and "image_url" not in m:
            raise InvalidRequestError(message="tin nhắn: thiếu nội dung", param='messages')
...
def completions_common(body: dict, is_legacy: bool = False, stream=False):
    ...
    if prompt_str not in body:
        raise InvalidRequestError("Thiếu thông tin đầu vào bắt buộc", param=prompt_str)
    ...
    if not stream:
        ...
        if isinstance(prompt, list):
            if prompt and isinstance(prompt[0], int):
                ...
                raise InvalidRequestError(message="Việc tạo hàng loạt API chưa được hỗ trợ.", param=prompt_str)
