"""
Mô-đun này chịu trách nhiệm về API VectorDB. Nó hiện hỗ trợ:
* XÓA api/v1/clear
    - Xóa toàn bộ DB.
* ĐĂNG api/v1/thêm
    - Thêm một số kho văn bản vào DB. Bạn cũng có thể chỉ định siêu dữ liệu sẽ được thêm vào cùng với nó.
* ĐĂNG api/v1/xóa
    - Xóa các bản ghi cụ thể với siêu dữ liệu nhất định.
* ĐĂNG api/v1/get
    - Nhận kết quả từ chromaDB.
"""
    ...
    def do_POST(self):
        try:
            ...
            if path in ['/api/v1/add', '/api/add']:
                ...
                if corpus is None:
                    self._send_412_error("Thiếu tham số 'corpus'")
                    return
                ...
                self._send_200_response("Đã thêm dữ liệu thành công")

            elif path in ['/api/v1/delete', '/api/delete']:
                metadata = body.get('metadata')
                if corpus is None:
                    self._send_412_error("Thiếu tham số 'siêu dữ liệu'")
                    return
                
                self.collector.delete(ids_to_delete=None, where=metadata)
                self._send_200_response("Dữ liệu đã được xóa thành công")

            elif path in ['/api/v1/get', '/api/get']:
                search_strings = body.get('search_strings')
                if search_strings is None:
                    self._send_412_error("Thiếu tham số 'search_strings'")
                    return
    ...
    def do_DELETE(self):
        try:
            ...
            if path in ['/api/v1/clear', '/api/clear']:
                ...
                self._send_200_response("Đã xóa dữ liệu thành công")
            else:
                self._send_404_error()
        except Exception as e:
            self._send_400_error(str(e))
...
class APIManager:
    ...
    def stop_server(self):
        if self.server is not None:
            logger.info(f'Dừng API chromaDB.')
