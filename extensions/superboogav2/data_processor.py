"""
Mô-đun này chịu trách nhiệm xử lý kho dữ liệu và đưa nó vào chromaDB. Nó sẽ nhận được một tập văn bản.
Sau đó nó sẽ chia nó thành các phần có độ dài được chỉ định. Đối với mỗi phần đó, nó sẽ nối thêm bối cảnh xung quanh.
Nó sẽ chỉ bao gồm các từ đầy đủ.
"""
...
def _create_chunks_with_context(corpus, chunk_len, context_left, context_right):
    """
    Hàm này lấy một tập văn bản và chia nó thành các phần có độ dài xác định,
    sau đó thêm một lượng ngữ cảnh cụ thể vào từng đoạn. Bối cảnh được thêm vào đầu tiên
    đi lùi từ đầu đoạn và sau đó đi tiếp từ cuối đoạn
    đoạn, đảm bảo rằng ngữ cảnh chỉ bao gồm toàn bộ từ và tổng chiều dài ngữ cảnh
    không vượt quá giới hạn quy định. Hàm này sử dụng tìm kiếm nhị phân để đạt hiệu quả.

    Trả về:
    chunks (danh sách str): Các đoạn văn bản.
    chunks_with_context (danh sách str): Các đoạn văn bản có thêm ngữ cảnh.
    chunk_with_context_start_indices (danh sách int): Chỉ số bắt đầu của mỗi đoạn có ngữ cảnh trong kho văn bản.
    """
...
def process_and_add_to_collector(corpus: str, collector: ChromaCollector, clear_collector_before_adding: bool, metadata: dict):
    ...
    if len(context_len) >= 3:
        raise f"Len bối cảnh có quá nhiều giá trị: {len(context_len)}"
