"""
Tệp này chịu trách nhiệm về giao diện người dùng và cách ứng dụng tương tác với phần còn lại của hệ thống.
"""
import os
...
def _begin_benchmark():
    ...
    return f'**Điểm số**: {score}/{max_score}'
...
def _apply_settings(optimization_steps, time_power, time_steepness, significant_level, min_sentences, new_dist_strat, delta_start, min_number_length, num_conversion, 
    ...
    try:
        ...
        logger.warn(f'Không thể áp dụng đúng cài đặt: {str(e)}')
...
def ui():
    with gr.Accordion("Bấm để biết thêm thông tin...", open=False):
    ...
    with gr.Row():
        with gr.Column(min_width=600):
            with gr.Tab("Nhập văn bản vào"):
                data_input = gr.Textbox(lines=20, label='Nhập dữ liệu vào')
                update_data = gr.Button('Tải dữ liệu')

            with gr.Tab("Nhập URL vào"):
                url_input = gr.Textbox(lines=10, label='Đầu vào URLs', info='Nhập một hoặc nhiều URL được phân tách bằng ký tự dòng mới.')
            ...
            with gr.Tab("Settings"):
                ...
                with gr.Accordion("Advanced settings", open=False):
                    ...
                    preprocess_pipeline = gr.CheckboxGroup(label='Sơ chế pipeline', choices=[
                        'Lower Cases',
                        'Remove Punctuation',
                        'Remove Adverbs',
                        'Remove Stop Words',
                        ...
                        'Merge Spaces',
                        'Strip Edges',
                    ], value=preprocess_set_choices, interactive=True, info='Cách xử lý trước văn bản trước khi chuyển sang dạng nhúng.')
                    ...
                    with gr.Row():
                        ...
                        min_number_length = gr.Number(value=parameters.get_min_num_length(), label='Ngưỡng độ dài số', info='In digits. Only numbers that have at least that many digits will be converted.', interactive=True)

                    delta_start = gr.Number(value=parameters.get_delta_start(), label='Chỉ số khởi đầu Delta', info='Nếu hệ thống gặp hai phần nhúng giống hệt nhau và cả hai đều bắt đầu trong cùng một delta thì chỉ phần đầu tiên sẽ được xem xét.', interactive=True)
                    new_dist_strat = gr.Dropdown(choices=[parameters.DIST_MIN_STRATEGY, parameters.DIST_HARMONIC_STRATEGY, parameters.DIST_GEOMETRIC_STRATEGY, parameters.DIST_ARITHMETIC_STRATEGY], value=parameters.get_new_dist_strategy(), label="Chiến lược khoảng cách", info='Khi hai văn bản nhúng được hợp nhất, khoảng cách của phần mới sẽ được quyết định bằng một trong các chiến lược sau.', interactive=True)
                    min_sentences = gr.Number(value=parameters.get_min_num_sentences(), label='Ngưỡng tóm tắt', info='Trong câu. Số lượng câu tối thiểu để kích hoạt tóm tắt thứ hạng văn bản.', interactive=True)
                    significant_level = gr.Slider(0.8, 2, value=parameters.get_significant_level(), label='Mức độ đáng kể', info='Xác định giới hạn cho khoảng cách được coi là "đáng kể" so với khoảng cách trung bình giữa các mẫu được trả về.', interactive=True)
                    time_steepness = gr.Slider(0.01, 1.0, value=parameters.get_time_steepness(), label='Thời gian cân độ dốc', info='Hai đoạn trích gần nhau sẽ được cân nhắc khác nhau như thế nào.')
                    time_power = gr.Slider(0.0, 1.0, value=parameters.get_time_power(), label='Sức mạnh cân thời gian', info='How influencial is the weighing. At 1.0, old entries won\'t be considered')

            with gr.Tab("Điểm chuẩn"):
                benchmark_button = gr.Button('Điểm chuẩn')
                optimize_button = gr.Button('Tối ưu hóa')
                optimization_steps = gr.Number(value=parameters.get_optimization_steps(), label='Các bước tối ưu hóa', info='Có bao nhiêu bước để tối ưu hóaCó bao nhiêu bước để tối ưu hóa.', interactive=True)


            clear_button = gr.Button('❌ Xóa dữ liệu')
