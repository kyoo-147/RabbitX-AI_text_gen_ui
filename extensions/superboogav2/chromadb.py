...
logger.debug('Chặn tất cả các cuộc gọi đến posthog.')
...
class ChromaCollector(Collecter):
    ...
    def add(self, texts: list[str], texts_with_context: list[str], starting_indices: list[int], metadatas: list[dict] = None):
        with self.lock:
            assert metadatas is None or len(metadatas) == len(texts), "siêu dữ liệu phải là Không có hoặc có cùng độ dài với văn bản"
            ...
            if existing_texts:
                logger.info(f'Đã thêm {len(existing_embeddings)} nhúng được lưu trong bộ nhớ đệm.')
            ...
            if non_existing_texts:
                ...
                logger.info(f'Đang thêm {len(non_existing_embeddings)} phần nhúng mới.')
    ...
    def delete(self, ids_to_delete: list[str], where: dict):
        with self.lock:
            ...
            logger.info(f'Đã xóa thành công {len(ids_to_delete)} bản ghi từ chromaDB.')
    ...
    def clear(self):
        with self.lock:
            ...
            logger.info('Đã xóa thành công tất cả bản ghi và đặt lại chromaDB.')


class SentenceTransformerEmbedder(Embedder):
    def __init__(self) -> None:
        logger.debug('Tạo câu nhúng...')
