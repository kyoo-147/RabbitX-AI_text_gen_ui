"""
Mô-đun này cung cấp một lớp đơn `Tham số` được sử dụng để quản lý tất cả các siêu tham số cho ứng dụng nhúng.
Nó mong đợi một tệp JSON trong `extensions/superboogav2/config.json`.

Mỗi phần tử trong JSON phải có giá trị `default` sẽ được sử dụng cho lần chạy hiện tại. Các phần tử có thể có `danh mục`.
Các danh mục này xác định phạm vi mà trình tối ưu hóa sẽ tìm kiếm. Nếu phần tử được gắn thẻ `" Should_optimize ": false`,
thì trình tối ưu hóa sẽ chỉ sử dụng giá trị mặc định.
"""
...
class Parameters:
    ...
    def __init__(self):
        if Parameters._instance is not None:
            raise Exception("Lớp học này là một lớp đơn!")
        else:
    ...
    def _load_from_json(self, file_path):
        logger.debug('Đang tải siêu tham số...')
...
def set_chunk_len(val: str):
    ...
    if len(chunk_lens) > 4:
        logger.warning(f'Đang tải siêu tham số {chunk_lens[4:]}')
...
def set_context_len(val: str):
    ...
    if len(context_lens) == 1:
        ...
        logger.warning(f'Đã nhận được độ dài ngữ cảnh không chính xác {val}. Bỏ qua.')
