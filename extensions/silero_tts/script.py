...
def load_model():
    ...
    if Path(model_path).is_file():
        print(f'\nSử dụng điểm kiểm tra được lưu trong bộ nhớ cache Silero TTS được tìm thấy tại {torch_cache_path}')
        ...
        print(f'\nKhông tìm thấy bộ đệm Silero TTS tại {torch_cache_path}. Đang cố gắng tải xuống...')
...
def input_modifier(string, state):
    ...
    shared.processing_message = "*Đang ghi âm tin nhắn thoại...*"
    return string
...
def output_modifier(string, state):
    ...
    if string == '':
        string = '*Trả lời trống, hãy thử tạo lại*'
    else:
    ...
    shared.processing_message = "*Đang nhập...*"
    return string
...
def ui():
    ...
    with gr.Accordion("Silero TTS"):
        with gr.Row():
            activate = gr.Checkbox(value=params['activate'], label='Kích hoạt TTS')
            autoplay = gr.Checkbox(value=params['autoplay'], label='Tự động phát TTS')

        show_text = gr.Checkbox(value=params['show_text'], label='Hiển thị văn bản tin nhắn dưới trình phát âm thanh')
        
        with gr.Row():
            language = gr.Dropdown(value=params['language'], choices=sorted(languages.keys()), label='Ngôn ngữ')
            voice = gr.Dropdown(value=params['speaker'], choices=languages[params['language']]["voices"], label='Giọng nói TTS')
        with gr.Row():
            v_pitch = gr.Dropdown(value=params['voice_pitch'], choices=voice_pitches, label='Cao độ giọng nói')
            v_speed = gr.Dropdown(value=params['voice_speed'], choices=voice_speeds, label='Tốc độ giọng nói')

        with gr.Row():
            preview_text = gr.Text(show_label=False, placeholder="Xem trước văn bản", elem_id="silero_preview_text")
            preview_play = gr.Button("Xem trước")
            preview_audio = gr.HTML(visible=False)

        with gr.Row():
            convert = gr.Button('Thay thế vĩnh viễn âm thanh bằng tin nhắn')
            convert_cancel = gr.Button('Hủy bỏ', visible=False)
            convert_confirm = gr.Button('Xác nhận (Không thể hoàn tác)', variant="stop", visible=False)
