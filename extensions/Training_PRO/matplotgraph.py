...
def create_graph(lora_path, lora_name):
    try:
        ...
        if os.path.exists(peft_model_path):
            ...
            print(f"Đồ thị được lưu trong {image_model_path}")
        else:
            print(f"File 'training_graph.json' không tồn tại trong {lora_path}")
      
    except ImportError:
        print("matplotlib chưa được cài đặt. Hãy cài đặt matplotlib để tạo đồ thị PNG")