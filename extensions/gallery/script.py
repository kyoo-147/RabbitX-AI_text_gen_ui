...
def generate_css():
    css = """
      .highlighted-border {
        border-color: rgb(4, 174, 4) !important;
      }
...
def ui():
    with gr.Accordion("Thư viện nhân vật", open=settings["gallery-open"], elem_id='gallery-extension'):
        gr.HTML(value="<style>" + generate_css() + "</style>")
        with gr.Row():
            filter_box = gr.Textbox(label='', placeholder='Lọc', lines=1, max_lines=1, container=False, elem_id='gallery-filter-box')
            gr.ClearButton(filter_box, value='Clear', elem_classes='refresh-button')
            update = gr.Button("Làm mới", elem_classes='refresh-button')
