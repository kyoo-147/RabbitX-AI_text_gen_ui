...
def setup():
    global model
    print("[XTTS] Đang tải XTTS...")
    model = load_model()
    print("[XTTS] Hoàn tất quá trình!")
...
def ui():
    with gr.Accordion("Coqui TTS (XTTSv2)"):
        ...
        with gr.Row():
            ...
            remove_trailing_dots = gr.Checkbox(value=params['remove_trailing_dots'], label='Xóa dấu vết "." từ các đoạn văn bản trước khi chuyển đổi sang âm thanh')

        with gr.Row():
            with gr.Row():
                voice = gr.Dropdown(get_available_voices(), label="Âm thanh wav", value=params["voice"])
                create_refresh_button(voice, lambda: None, lambda: {'choices': get_available_voices(), 'value': params["voice"]}, 'refresh-button')

            language = gr.Dropdown(languages.keys(), label="Ngôn ngữ", value=params["language"])
