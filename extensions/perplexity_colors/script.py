...
class PerplexityLogits(LogitsProcessor):
    ...
    def __call__(self, input_ids, scores):
        ...
        if len(self.selected_probs) > 0:
            ...
            if self.verbose:
                print('Probs: Mã thông báo sau', shared.tokenizer.decode(last_token_id))
        ...
        if self.verbose:
            ...
            print(f"PPL: Mã thông báo sau {shared.tokenizer.decode(last_token_id)}\t{perplexity:.2f}\t{pplbar}")
...
def output_modifier(text):
    ...
    print('Độ bối rối trung bình:', round(np.mean(ppl_logits_processor.perplexities_list[:-1]), 4))
    # t1 = time.time()
...
def perplexity_color_scale(ppl):
    '''
    Chỉ thành phần màu đỏ, màu trắng cho độ bối rối 0 (xin lỗi nếu bạn không ở chế độ tối)
    '''
...
def probability_perplexity_color_scale(prob, ppl):
    '''
    Xanh-vàng-đỏ cho xác suất và thành phần xanh lam cho sự bối rối
    '''
...
def ui():
    ...
    active_check = gradio.Checkbox(value=True, label="Tính toán xác suất và điểm số phức tạp", info="Kích hoạt tiện ích mở rộng này. Lưu ý rằng tiện ích mở rộng này hiện không hoạt động với exllama hoặc llama.cpp.")
    color_by_ppl_check = gradio.Checkbox(value=False, label="Màu sắc bởi sự bối rối", info="Sự bối rối càng cao thì màu đỏ càng nhiều. Nếu cũng hiển thị xác suất, độ phức tạp cao hơn có nhiều thành phần màu xanh hơn.")
    color_by_prob_check = gradio.Checkbox(value=False, label="Màu theo xác suất", info="Thang tuyến tính xanh-vàng-đỏ, với 100% xanh lục, 50% vàng, 0% đỏ.")
    prob_dropdown_check = gradio.Checkbox(value=False, label="Xác suất thả xuống", info="Di chuột qua mã thông báo để hiển thị danh sách thả xuống các xác suất mã thông báo hàng đầu. Hiện tại hơi có lỗi với khoảng trắng giữa các mã thông báo.")
