...
def tokenizer_modifier(state, prompt, input_ids, input_embeds):
    ...
    logger.info(f'Đã nhúng {total_embedded} (các) hình ảnh trong {time.time()-start_ts:.2f}s')
    return (prompt,
...
def ui():
    ...
    with gr.Column():
        picture_select = gr.Image(label='Gửi hình ảnh', type='pil')
        # The models don't seem to deal well with multiple images
        single_image_checkbox = gr.Checkbox(False, label='Nhúng tất cả hình ảnh, không chỉ hình ảnh cuối cùng')
