...
class LLaVA_v0_Pipeline(AbstractMultimodalPipeline):
    ...
    def _load_models(self):
        ...
        logger.info(f"LLaVA - Đang tải CLIP từ {self.CLIP_REPO} bằng {self.clip_dtype} trên {self.clip_device}...")
        ...
        logger.info(f"LLaVA - Đang tải máy chiếu từ {self.llava_projector_repo()} bằng {self.projector_dtype} trên {self.projector_device}...")
        ...
        logger.info(f"LLaVA mô hình hỗ trợ được tải, lấy {time.time() - start_ts:.2f} giây")
    ...
    def embed_tokens(input_ids: torch.Tensor) -> torch.Tensor:
        for attr in ['', 'model', 'model.model', 'model.model.model']:
            ...
            raise ValueError('Không tìm thấy phương thức embed_tokens cho trình tải này.')
