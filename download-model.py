'''
Tải các mô hình từ Ôm Mặt xuống models/username_modelname.
...
class ModelDownloader:
    ...
    def sanitize_model_and_branch_names(self, model, branch):
        ...
        if branch is None:
            ...
            if not pattern.match(branch):
                raise ValueError(
                    "Tên chi nhánh không hợp lệ. Chỉ cho phép các ký tự chữ và số, dấu chấm, dấu gạch dưới và dấu gạch ngang.")
    ...
    def check_model_files(self, model, branch, links, sha256, output_folder):
        ...
        for i in range(len(sha256)):
            ...
            if not fpath.exists():
                print(f"Tệp sau bị thiếu: {fpath}")
                validated = False
                continue
            ...
            with open(output_folder / sha256[i][0], "rb") as f:
                ...
                if file_hash != sha256[i][1]:
                    print(f'Tổng kiểm tra không thành công: {sha256[i][0]}  {sha256[i][1]}')
                    validated = False
                else:
                    print(f'Tổng kiểm tra đã được xác thực: {sha256[i][0]}  {sha256[i][1]}')

        if validated:
            print('[+] Tổng kiểm tra được xác thực của tất cả các tệp mô hình!')
        else:
            print('[-] Tổng kiểm tra không hợp lệ. Chạy lại download-model.py với --clean flag.')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--branch', type=str, default='main', help='Tên của nhánh Git để tải xuống từ.')
    parser.add_argument('--threads', type=int, default=4, help='Số lượng file tải xuống đồng thời.')
    parser.add_argument('--text-only', action='store_true', help='Chỉ tải xuống các tập tin văn bản (txt/json).')
    parser.add_argument('--specific-file', type=str, default=None, help='Tên của tệp cụ thể cần tải xuống (nếu không được cung cấp, hãy tải xuống tất cả).')
    parser.add_argument('--output', type=str, default=None, help='Thư mục nơi mô hình sẽ được lưu.')
    parser.add_argument('--clean', action='store_true', help='Không tiếp tục tải xuống trước đó.')
    parser.add_argument('--check', action='store_true', help='Xác thực tổng kiểm tra của các tệp mô hình.')
    parser.add_argument('--max-retries', type=int, default=5, help='Số lần thử lại tối đa khi gặp lỗi trong thời gian tải xuống.')
    ...
    if model is None:
        print("Lỗi: Vui lòng chỉ định model bạn muốn tải xuống (e.g. 'python download-model.py facebook/opt-1.3b').")
        sys.exit()
