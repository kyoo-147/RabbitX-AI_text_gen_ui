...
def check_env():
    ...
    if not conda_exist:
        print("Conda dã không được cài đặt. Đang thoát...")
        sys.exit(1)

    # Ensure this is a new environment and not the base environment
    if os.environ["CONDA_DEFAULT_ENV"] == "base":
        print("Tạo một môi trường trong dự án và kích hoạt nó. Đang thoát...")
        sys.exit(1)
...
def install_webui():
    ...
    if "GPU_CHOICE" in os.environ:
        ...
        print_big_message(f"Lựa chọn GPU đã chọn \"{choice}\" dựa trên biến môi trường GPU_CHOICE.")
    else:
        print()
        print("GPU của bạn là gì?")
        print()
        print("A) NVIDIA")
        print("B) AMD (Chỉ Linux/MacOS. Yêu cầu ROCm SDK 5.6 trên Linux)")
        print("C) Apple M Series")
        print("D) Intel Arc (IPEX)")
        print("N) Không (Tôi muốn sử dụng chế độ CPU)")
        print()

        choice = input("Lựa chọn> ").upper()
        while choice not in 'ABCDN':
            print("Lựa chọn không đúng. Vui lòng thử lại.")
            choice = input("Lựa chọn> ").upper()
    ...
    if selected_gpu == "NONE":
        with open(cmd_flags_path, 'r+') as cmd_flags_file:
            if "--cpu" not in cmd_flags_file.read():
                print_big_message("Thêm cờ --cpu đến CMD_FLAGS.txt.")
    ...
    if any((is_windows(), is_linux())) and selected_gpu == "NVIDIA":
        if "USE_CUDA118" in os.environ:
            ...
            print("\nBạn có muốn sử dụng CUDA 11.8 thay vì 12.1 không? Chỉ chọn tùy chọn này nếu GPU của bạn quá cũ (Kepler trở xuống).\nĐối với GPU dòng RTX và GTX, hãy nói \"N\". Nếu không chắc chắn, hãy nói \"N\".\n")
            use_cuda118 = input("Lựa chọn (Y/N)> ").upper().strip('"\'').strip()
            while use_cuda118 not in 'YN':
                print("Lựa chọn không đúng. Vui lòng thử lại.")
                use_cuda118 = input("Lựa chọn> ").upper().strip('"\'').strip()
        ...
        if is_linux():
            ...
            print("AMD GPUs chỉ hỗ trợ trên Linux. Đang thoát ...")
            sys.exit(1)
    ...
    print_big_message("Đang cài đặt PyTorch.")
    ...
    if selected_gpu == "NVIDIA":
        print_big_message("Đang cài đặt thư viện CUDA thời gian thực.")
    ...
    if selected_gpu == "INTEL":
        ...
        print_big_message("Đang cài đặt thư viện Intel oneAPI thời gian thực.")
...
def update_requirements(initial_installation=False):
    ...
    if install:
        print_big_message("Đang cài đặt tiện ích yêu cầu.")
        ...
        print_big_message("Sẽ không cài đặt tiện ích từ biến môi trường INSTALL_EXTENSIONS.")
    ...
    print_big_message(f"Đang cài đặt giao diện web yêu cầu từ tệp: {requirements_file}")
    ...
    for req in git_requirements:
        ...
        print(f"Đã gỡ cài đặt {package_name}")
...
if __name__ == "__main__":
    ...
    parser.add_argument('--update', action='store_true', help='Cập nhập giao diện web.')
    ...
    if args.update:
        ...
        if os.environ.get("LAUNCH_AFTER_INSTALL", "").lower() in ("no", "n", "false", "0", "f", "off"):
            print_big_message("Cài đặt kết thúc thành công và sẽ thoát khỏi LAUNCH_AFTER_INSTALL.")
            sys.exit()
        ...
        if len([item for item in glob.glob(f'{model_dir}/*') if not item.endswith(('.txt', '.yaml'))]) == 0:
            print_big_message("CẢNH BÁO: Bạn chưa tải xuống bất kỳ mô hình nào.\nSau khi giao diện người dùng web khởi chạy, hãy chuyển tới tab \"Mẫu\" và tải xuống một mô hình.")
