...
def signal_handler(sig, frame):
    logger.info("Đã nhận được Ctrl+C. Tắt giao diện người dùng web tạo văn bản một cách duyên dáng.")
    sys.exit(0)
...
if __name__ == "__main__":
    ...
    # Select the model from a command-line menu
        ...
        if len(available_models) == 0:
            logger.error('Không có mô hình có sẵn! Vui lòng tải xuống ít nhất một.')
            sys.exit(0)
        else:
