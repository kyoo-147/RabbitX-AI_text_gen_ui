...
def create_ui():
    ...
    with gr.Tab('Mặc định', elem_id='default-tab'):
        shared.gradio['last_input-default'] = gr.State('')
        with gr.Row():
            with gr.Column():
                with gr.Row():
                    shared.gradio['textbox-default'] = gr.Textbox(value='', lines=27, label='Đầu vào', elem_classes=['textbox_default', 'add_scrollbar'])
                    shared.gradio['token-counter-default'] = gr.HTML(value="<span>0</span>", elem_classes=["token-counter", "default-token-counter"])

                with gr.Row():
                    shared.gradio['Generate-default'] = gr.Button('Khởi tạo', variant='primary')
                    shared.gradio['Stop-default'] = gr.Button('Dừng', elem_id='stop')
                    shared.gradio['Continue-default'] = gr.Button('Tiếp tục')
            ...
            with gr.Column():
                with gr.Tab('Raw'):
                    shared.gradio['output_textbox'] = gr.Textbox(lines=27, label='Đầu ra', elem_id='textbox-default', elem_classes=['textbox_default_output', 'add_scrollbar'])

                with gr.Tab('Markdown'):
                    shared.gradio['markdown_render-default'] = gr.Button('Kết xuất')
                ...
                with gr.Tab('Nhật ký'):
                    with gr.Row():
                        with gr.Column(scale=10):
                            shared.gradio['get_logits-default'] = gr.Button('Nhận xác suất mã thông báo tiếp theo')
                        with gr.Column(scale=1):
                            shared.gradio['use_samplers-default'] = gr.Checkbox(label='Sử dụng dụng cụ lấy mẫu', value=True, elem_classes=['no-background'])

                    with gr.Row():
                        shared.gradio['logits-default'] = gr.Textbox(lines=23, label='Đầu ra', elem_classes=['textbox_logits', 'add_scrollbar'])
                        shared.gradio['logits-default-previous'] = gr.Textbox(lines=23, label='Đầu ra trước đó', elem_classes=['textbox_logits', 'add_scrollbar'])

                with gr.Tab('Tokens'):
                    shared.gradio['get_tokens-default'] = gr.Button('Nhận ID mã thông báo cho đầu vào')
