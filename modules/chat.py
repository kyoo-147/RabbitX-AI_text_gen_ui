...
def str_presenter(dumper, data):
    """
    Sao chép từ https://github.com/yaml/pyyaml/issues/240
    Làm cho đầu ra pyyaml có nhiều dòng đẹp hơn.
    """
...
def get_generation_prompt(renderer, impersonate=False, strip_trailing_spaces=True):
    '''
    Với mẫu Jinja, hãy thiết kế ngược tiền tố và hậu tố cho
    một tin nhắn hỗ trợ (impersonate=False) hoặc một tin nhắn của người dùng
    (if impersonate=True)
    '''
    ...
    if shared.tokenizer is not None:
        ...
        while len(messages) > 0 and encoded_length > max_length:
            ...
            # Resort to truncating the user input
                ...
                encoded_length = get_encoded_length(prompt)
                if encoded_length > max_length:
                    logger.error(f"Không tạo được lời nhắc trò chuyện. Đầu vào quá dài so với độ dài ngữ cảnh có sẵn.\n\nChiều dài cắt ngắn: {state['truncation_length']}\nmax_new_tokens: {state['max_new_tokens']} (có cao quá không?)\nĐộ dài ngữ cảnh có sẵn: {max_length}\n")
                    raise ValueError
                else:
                    logger.warning(f"Đầu vào đã bị cắt ngắn. Độ dài ngữ cảnh: {state['truncation_length']}, max_new_tokens: {state['max_new_tokens']}, available context length: {max_length}.")
                    break
...
def chatbot_wrapper(text, state, regenerate=False, _continue=False, loading_message=True, for_ui=False):
    ...
    if shared.model_name == 'None' or shared.model is None:
        raise ValueError("Không có mô hình nào được tải! Chọn một trong tab Model.")
...
def impersonate_wrapper(text, state):
    ...
    if shared.model_name == 'None' or shared.model is None:
        logger.error("Không có mô hình nào được tải! Chọn một trong tab Model.")
        yield '', static_output
        return
...
def character_is_loaded(state, raise_exception=False):
    if state['mode'] in ['chat', 'chat-instruct'] and state['name2'] == '':
        logger.error('Có vẻ như không có ký tự nào được tải. Vui lòng tải một cái trong Tham số > Ký tự.')
        if raise_exception:
            raise ValueError
...
def generate_chat_reply_wrapper(text, state, regenerate=False, _continue=False):
    '''
    Tương tự như trên nhưng trả về HTML cho giao diện người dùng
    '''
...
def rename_history(old_id, new_id, character, mode):
    ...
    if new_p.parent != old_p.parent:
        logger.error(f"Đường dẫn sau không được phép: \"{new_p}\".")
    elif new_p == old_p:
        logger.info("Đường dẫn được cung cấp giống hệt với đường dẫn cũ.")
    else:
        logger.info(f"Đổi tên \"{old_p}\" thành \"{new_p}\"")
        old_p.rename(new_p)
...
def load_latest_history(state):
    '''
    Tải lịch sử mới nhất cho nhân vật đã cho trong trò chuyện hoặc hướng dẫn trò chuyện
    chế độ hướng dẫn hoặc lịch sử lệnh mới nhất cho chế độ lệnh.
    '''
...
def load_character(character, name1, name2):
    ...
    if filepath is None or not filepath.exists():
        logger.error(f"Không thể tìm thấy nhân vật \"{character}\" ký tự bên trong/. Không có ký tự nào được tải.")
        raise ValueError
...
def upload_character(file, img, tavern=False):
    ...
    logger.info(f'Ký tự mới được lưu vào "characters/{outfile_name}.yaml".')
...
def upload_your_profile_picture(img):
    ...
    if img is None:
        ...
        logger.info(f'Đã lưu ảnh hồ sơ vào "{cache_folder}/pfp_me.png"')
...
def save_character(name, greeting, context, picture, filename):
    if filename == "":
        logger.error("Tên file trống nên ký tự sẽ không được lưu.")
        return
    ...
    if picture is not None:
        ...
        logger.info(f'Đã lưu {path_to_img}.')
...
def my_yaml_output(data):
    '''
    pyyaml rất không phù hợp với chuỗi nhiều dòng.
    đối với đầu ra mẫu lệnh đơn giản, điều này là đủ.
    '''
    result = ""
    for k in data:
        result += k + ": |-\n"
