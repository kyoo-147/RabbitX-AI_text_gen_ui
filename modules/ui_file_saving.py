...
def create_ui():
    ...
    with gr.Group(visible=False, elem_classes='file-saver') as shared.gradio['file_saver']:
        shared.gradio['save_filename'] = gr.Textbox(lines=1, label='Tên tập tin')
        shared.gradio['save_root'] = gr.Textbox(lines=1, label='Thư mục tập tin', info='Để tham khảo. không thể thay đổi.', interactive=False)
        shared.gradio['save_contents'] = gr.Textbox(lines=10, label='Nội dung tập tin')
        with gr.Row():
            shared.gradio['save_cancel'] = gr.Button('Hủy bỏ', elem_classes="small-button")
            shared.gradio['save_confirm'] = gr.Button('Lưu', elem_classes="small-button", variant='primary', interactive=not mu)

    # Text file deleter
    with gr.Group(visible=False, elem_classes='file-saver') as shared.gradio['file_deleter']:
        shared.gradio['delete_filename'] = gr.Textbox(lines=1, label='Tên tập tin')
        shared.gradio['delete_root'] = gr.Textbox(lines=1, label='Thư mục tập tin', info='Để tham khảo. Không thể thay đổi.', interactive=False)
        with gr.Row():
            shared.gradio['delete_cancel'] = gr.Button('Hủy bỏ', elem_classes="small-button")
            shared.gradio['delete_confirm'] = gr.Button('Xóa', elem_classes="small-button", variant='stop', interactive=not mu)

    # Character saver/deleter
    with gr.Group(visible=False, elem_classes='file-saver') as shared.gradio['character_saver']:
        shared.gradio['save_character_filename'] = gr.Textbox(lines=1, label='Tên tập tin', info='Ký tự sẽ được lưu vào thư mục ký tự/của bạn với tên tệp cơ sở này.')
        with gr.Row():
            shared.gradio['save_character_cancel'] = gr.Button('Hủy bỏ', elem_classes="small-button")
            shared.gradio['save_character_confirm'] = gr.Button('Lưu', elem_classes="small-button", variant='primary', interactive=not mu)

    with gr.Group(visible=False, elem_classes='file-saver') as shared.gradio['character_deleter']:
        gr.Markdown('Xác nhận xóa ký tự?')
        with gr.Row():
            shared.gradio['delete_character_cancel'] = gr.Button('Hủy bỏ', elem_classes="small-button")
            shared.gradio['delete_character_confirm'] = gr.Button('Xóa', elem_classes="small-button", variant='stop', interactive=not mu)

    # Preset saver
    with gr.Group(visible=False, elem_classes='file-saver') as shared.gradio['preset_saver']:
        shared.gradio['save_preset_filename'] = gr.Textbox(lines=1, label='Tên tập tin', info='Giá trị đặt trước sẽ được lưu vào thư mục cài đặt trước/của bạn với tên tệp cơ sở này.')
        shared.gradio['save_preset_contents'] = gr.Textbox(lines=10, label='Nội dung tập tin')
        with gr.Row():
            shared.gradio['save_preset_cancel'] = gr.Button('Hủy bỏ', elem_classes="small-button")
            shared.gradio['save_preset_confirm'] = gr.Button('Lưu', elem_classes="small-button", variant='primary', interactive=not mu)
