'''
File này đã được copy 100% từ PR này vào thư viện Transformers:
https://github.com/huggingface/transformers/pull/27557

Tác giả: Saibo-creator
Tác giả GitHub: https://github.com/Saibo-creator

Tất cả các khoản tín dụng gửi đến tác giả.
'''
...
def remove_leading_white_space(src, newline_ok):
    """
    Bỏ qua khoảng trắng và nhận xét trong chuỗi đầu vào.
    Hàm này xử lý chuỗi đầu vào, bỏ qua mọi dấu cách, tab,
    và nội dung theo sau ký tự '#', biểu thị nhận xét. Việc phân tích cú pháp
    của một bình luận tiếp tục cho đến cuối dòng (được biểu thị bằng các ký tự dòng mới
    '\r' hoặc '\n'). Nếu tham số 'newline_ok' được đặt thành Sai, hàm
    sẽ ngừng xử lý và trả về chuỗi còn lại khi gặp phải
    ký tự dòng mới, nếu không nó cũng sẽ bỏ qua các ký tự dòng mới.
    Tham sô:
    src (str): Chuỗi đầu vào cần được xử lý.
    newline_ok (bool): Cờ cho biết có gặp ký tự dòng mới hay không
                       nên dừng phân tích cú pháp (Sai) hoặc nếu cần bỏ qua (Đúng).
    Trả về:
    str: Phần còn lại của chuỗi đầu vào sau khi bỏ qua khoảng trắng và chú thích.
    """
    pos = 0
...
def parse_char(src):
    """
    phân tích char hàng đầu từ chuỗi đầu vào
    :param src:
    ...
    if src[0] == "\\":
        ...
        if esc == "x":
            ...
            return "\t", src[2:]
        raise RuntimeError("chưa rõ lối thoát tại " + src)
    elif src:
        return src[0], src[1:]
    raise RuntimeError("cái kết không ngờ")
...
def parse_sequence(state, src, rule_name, outbuf, is_nested):
    ...
    while remaining_src:
        if remaining_src[0] == '"':  # literal string
            ...
            if remaining_src[0] != ")":
                raise RuntimeError("mong đợi ')' at " + remaining_src)
            remaining_src = remove_leading_white_space(remaining_src[1:], is_nested)
        elif remaining_src[0] in ("*", "+", "?"):  # repetition operator
            if len(outbuf) - out_start_pos - 1 == 0:
                raise RuntimeError("mong đợi mục trước đó */+/? tại " + remaining_src)
...
def parse_rule(state, src):
    ...
    if remaining_src and remaining_src[0] == "\r":
        ...
        raise RuntimeError("mong đợi dòng mới hoặc kết thúc lúc " + remaining_src)
...
def parse_ebnf(src):
    try:
        ...
        logger.warning("lỗi phân tích ngữ pháp:", err)
        return ParseState()
...
class GrammarConstraint(ABC):
    ...
    def accept_char(self, *args, **kwargs):
        """Xử lý một byte theo quy tắc ngữ pháp."""
        raise NotImplementedError

    def accept_token_id(self, *args, **kwargs):
        """Xử lý mã thông báo theo quy tắc ngữ pháp."""
        raise NotImplementedError
...
class IncrementalGrammarConstraint(GrammarConstraint):
    ...
    def accept_token_id(self, token_id: int, stacks: List[List[int]]):
        if token_id == self.eos_token_id:
            if stacks and all(len(stack) != 0 for stack in stacks):
                raise Exception(
                    f"Ít nhất một trong các ngăn xếp phải trống khi đạt đến EOS. Tuy nhiên, "
                    f"các ngăn xếp là {stacks}"
                )
            return []
    ...
    def filter_vocab(self, stacks, device):
        if not stacks:  # Check if stacks is empty
            ...
            logger.debug(f"tổng số chấp nhận: {0}")
        ...
        logger.debug(f"tổng số chấp nhận: {acceptance.sum()}")
        return acceptance
...
class TokenTrie:
    ...
    def load_tokens(self, tokenizer):
        ...
        if "gpt2" in tokenizer.__class__.__name__.lower():
            ...
            print("Cảnh báo: mã thông báo không được nhận dạng: sử dụng định dạng mã thông báo mặc định")
