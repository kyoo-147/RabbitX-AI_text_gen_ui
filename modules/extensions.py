...
def load_extensions():
    ...
    for i, name in enumerate(shared.args.extensions):
        if name in available_extensions:
            if name != 'api':
                logger.info(f'Đang tải tiện ích mở rộng "{name}"')
            try:
                try:
                    extension = importlib.import_module(f"extensions.{name}.script")
                except ModuleNotFoundError:
                    logger.error(f"Không thể nhập các yêu cầu cho '{name}'. Make sure to install the requirements for the extension.\n\nLinux / Mac:\n\npip install -r extensions/{name}/requirements.txt --upgrade\n\nWindows:\n\npip install -r extensions\\{name}\\requirements.txt --upgrade\n\nNếu bạn đã sử dụng trình cài đặt bằng một cú nhấp chuột, hãy dán lệnh trên vào cửa sổ terminal đã mở sau khi khởi chạy tập lệnh cmd cho hệ điều hành của bạn.")
                    raise
                ...
                logger.error(f'Không thể tải tiện ích mở rộng "{name}".')
                traceback.print_exc()
...
def apply_extensions(typ, *args, **kwargs):
    if typ not in EXTENSION_MAP:
        raise ValueError(f"Loại tiện ích mở rộng không hợp lệ {typ}")
