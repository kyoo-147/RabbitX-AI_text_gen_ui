...
def get_next_logits(prompt, state, use_samplers, previous, top_logits=25, return_dict=False):
    if shared.model is None:
        ...
        return 'Lỗi: Không có mô hình nào được tải1 Chọn một mô hình trong tab Mô hình.', previous
    ...
    if use_samplers:
        if any([is_non_hf_exllamav2, is_non_hf_llamacpp]):
            logger.error("Việc chiếm quyền điều khiển sampler không được hỗ trợ bởi các trình tải không phải Huggingface.")
            ...
            return 'Lỗi: Việc chiếm quyền điều khiển sampler không được hỗ trợ bởi các trình tải không phải Huggingface. Vui lòng tắt tùy chọn "Sử dụng mẫu".', previous
