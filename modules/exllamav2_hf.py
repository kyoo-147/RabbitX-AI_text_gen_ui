...
try:
    ...
    logger.warning(
        'Bạn đang chạy ExLlamaV2 mà không chú ý đến flash. Điều này sẽ gây ra việc sử dụng VRAM '
        'cao hơn rất nhiều so với mức có thể.\n'
        'Hãy thử cài đặt flash-attention theo hướng dẫn tại đây: '
        'https://github.com/Dao-AILab/flash-attention#installation-and-features'
    )
    pass
except Exception:
    logger.warning('Không thể tải chú ý flash do lỗi sau:\n')
    traceback.print_exc()
    ...
    def __call__(self, *args, **kwargs):
        ...
        if len(args) > 0:
            if not shared.args.cfg_cache:
                logger.error("Vui lòng kích hoạt tùy chọn cfg-cache để sử dụng CFG với ExLlamav2_HF.")
                return
