...
def update_model_parameters(state, initial=False):
    '''
    Giao diện người dùng: cập nhật các đối số dòng lệnh dựa trên các giá trị giao diện
    '''
...
def apply_model_settings_to_state(model, state):
    '''
    Giao diện người dùng: cập nhật biến trạng thái với cài đặt mô hình
    '''
...
def save_model_settings(model, state):
    '''
    Lưu cài đặt cho mô hình này vào models/config-user.yaml
    '''
    if model == 'None':
        yield ("Không lưu cài đặt vì không chọn model nào trong menu.")
        return
    ...
    yield (f"Cài đặt cho `{model}` được lưu tại `{p}`.")


def save_instruction_template(model, template):
    '''
    Tương tự như hàm trên nhưng nó chỉ lưu mẫu lệnh.
    '''
    if model == 'None':
        yield ("Không được lưu trong mẫu bởi vì không có mô hình được chọn trong danh sách.")
        return
    ...
    if template == 'None':
        yield (f"Mẫu hướng dẫn cho  `{model}` bỏ đặt trong `{p}`, vì giá trị của mẫu là `{template}`.")
    else:
        yield (f"Mẫu hướng dẫn cho `{model}` đã lưu tại `{p}` bằng `{template}`.")
