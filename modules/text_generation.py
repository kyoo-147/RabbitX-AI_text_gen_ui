...
def _generate_reply(question, state, stopping_strings=None, is_chat=False, escape_html=False, for_ui=False):
    ...
    if generate_func is None:
        if shared.model_name == 'None' or shared.model is None:
            logger.error("Không có mô hình được tải! Chọn một mô hình nào đó trong tab Mô hình.")
            yield ''
            return
...
def encode(prompt, add_special_tokens=True, add_bos_token=True, truncation_length=None):
    if shared.tokenizer is None:
        raise ValueError('Không có tokenizer được tải')
...
def generate_reply_HF(question, original_question, seed, state, stopping_strings=None, is_chat=False):
    ...
    try:
        ...
        print(f'Đầu ra được tạp ra trong {(t1-t0):.2f} giây ({new_tokens/(t1-t0):.2f} tokens/s, {new_tokens} tokens, context {original_tokens}, seed {seed})')
        return
...
def generate_reply_custom(question, original_question, seed, state, stopping_strings=None, is_chat=False):
    ...
    try:
        ...
        print(f'Đầu ra được tạo ra trong {(t1-t0):.2f} giây ({new_tokens/(t1-t0):.2f} tokens/s, {new_tokens} tokens, context {original_tokens}, seed {seed})')
        return
