...
def create_ui():
    ...
    with gr.Tab('Sổ ghi chép', elem_id='notebook-tab'):
        ...
        with gr.Row():
            with gr.Column(scale=4):
                ...
                with gr.Tab('Đánh dấu'):
                    shared.gradio['markdown_render-notebook'] = gr.Button('Kết xuất')
                ...
                with gr.Tab('Nhật ký'):
                    with gr.Row():
                        with gr.Column(scale=10):
                            shared.gradio['get_logits-notebook'] = gr.Button('Nhận xác suất mã thông báo tiếp theo')
                        with gr.Column(scale=1):
                            shared.gradio['use_samplers-notebook'] = gr.Checkbox(label='Use samplers', value=True, elem_classes=['no-background'])

                    with gr.Row():
                        shared.gradio['logits-notebook'] = gr.Textbox(lines=23, label='Đầu ra', elem_classes=['textbox_logits_notebook', 'add_scrollbar'])
                        shared.gradio['logits-notebook-previous'] = gr.Textbox(lines=23, label='Đầu ra trước đó', elem_classes=['textbox_logits_notebook', 'add_scrollbar'])

                with gr.Tab('Tokens'):
                    shared.gradio['get_tokens-notebook'] = gr.Button('Nhận ID mã thông báo cho đầu vào')
                ...
                with gr.Row():
                    shared.gradio['Generate-notebook'] = gr.Button('Khởi tạo', variant='primary', elem_classes='small-button')
                    shared.gradio['Stop-notebook'] = gr.Button('Dừng', elem_classes='small-button', elem_id='stop')
                    shared.gradio['Undo'] = gr.Button('Hoàn tác', elem_classes='small-button')
                    shared.gradio['Regenerate-notebook'] = gr.Button('Tái khởi tạo', elem_classes='small-button')
                    

            with gr.Column(scale=1):
                gr.HTML('<div style="padding-bottom: 13px"></div>')
                with gr.Row():
                    shared.gradio['prompt_menu-notebook'] = gr.Dropdown(choices=utils.get_available_prompts(), value='None', label='Lời nhắc', elem_classes='slim-dropdown')
