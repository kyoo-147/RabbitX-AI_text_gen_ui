...
class LlamacppHF(PreTrainedModel):
    ...
    def __call__(self, *args, **kwargs):
        ...
        if len(args) > 0:
            if not shared.args.cfg_cache:
                logger.error("Vui lòng kích hoạt tùy chọn cfg-cache để sử dụng CFG với llamacpp_HF.")
                return
    ...
    def from_pretrained(cls, pretrained_model_name_or_path: Optional[Union[str, os.PathLike]], *model_args, **kwargs):
        assert len(model_args) == 0 and len(kwargs) == 0, "đối số bổ sung hiện không được hỗ trợ"
        ...
        logger.info(f"Đã phát hiện trọng lượng llama.cpp: {model_file}\n")
