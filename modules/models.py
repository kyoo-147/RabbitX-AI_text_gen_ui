...
def load_model(model_name, loader=None):
    ...
    if loader is None:
        if shared.args.loader is not None:
            ...
            if loader is None:
                logger.error('Đường dẫn đến mô hình không tồn tại. Đang thoát.')
                raise ValueError
    ...
    logger.info(f"Đã tải mô hình vào {(time.time()-t0):.2f} giây.")
...
def load_tokenizer(model_name, model):
    ...
    if any(s in model_name.lower() for s in ['gpt-4chan', 'gpt4chan']) and Path(f"{shared.args.model_dir}/gpt-j-6B/").exists():
        ...
        if shared.args.no_use_fast:
            logger.info('Đang tải mã thông báo với use_fast=False.')
...
def huggingface_loader(model_name):
    ...
    # DeepSpeed ZeRO-3
        ...
        logger.info(f'DeepSpeed ZeRO-3 được cho phép: {is_deepspeed_zero3_enabled()}')

    # Load with quantization and/or offloading
    else:
        if not any((shared.args.cpu, torch.cuda.is_available(), is_xpu_available(), torch.backends.mps.is_available())):
            logger.warning('torch.cuda.is_available() và is_xpu_available() trả False. Điều này có nghĩa là không có GPU nào được phát hiện. Quay trở lại chế độ CPU.')
        ...
        if shared.args.cpu:
            ...
            if shared.args.load_in_4bit:
                ...
                logger.info('Sử dụng các thông số 4 bit sau: ' + str(quantization_config_params))
        ...
        if shared.args.disable_exllama or shared.args.disable_exllamav2:
            try:
                ...
                logger.info(f'Đang tải với disable_exllama={shared.args.disable_exllama} và disable_exllamav2={shared.args.disable_exllamav2}.')
            except:
                exc = traceback.format_exc()
                logger.error('Lỗi khi cho phép exllama, config.json cho mô hình này có chứa thông tin lượng tử hóa cần thiết không?')
                print(exc)
...
def llamacpp_HF_loader(model_name):
    ...
    if all((path / file).exists() for file in ['tokenizer_config.json']):
        logger.info(f'Sử dụng mã thông báo từ: \"{path}\"')
    else:
        logger.error("Không thể tải mô hình vì không tìm thấy mã thông báo ở định dạng Transformers.")
        return None, None
    ...
    if ctrans.model_type_is_auto():
        ...
        if path.is_file():
            ...
            if len(gguf) > 0:
                ...
                logger.error("Không thể tìm thấy mô hình cho máy biến áp ctransformers.")
                return None, None
...
def QuipSharp_loader(model_name):
    try:
        ...
        logger.error(
            "\nQuIP# vẫn chưa được tìm thấy. Bây giờ nó phải được cài đặt thủ công.\n"
            "Để được hướng dẫn cách thực hiện điều đó, vui lòng tham khảo:\n"
    ...
    if not all((model_dir / file).exists() for file in ['tokenizer_config.json', 'special_tokens_map.json', 'tokenizer.model']):
        logger.error(f"Không thể tải mô hình vì không thể tìm thấy tệp mã thông báo trong thư mục mô hình. Vui lòng tải các tệp sau từ mô hình gốc (không được lượng tử hóa) xuống {model_dir}: special_tokens_map.json, tokenizer.json, tokenizer.model, tokenizer_config.json.")
        return None, None
...
def GPTQ_loader(model_name):
    ...
    if shared.args.monkey_patch:
        logger.warning("Áp dụng bản vá khỉ để sử dụng LoRA với các mô hình GPTQ. Nó có thể gây ra hành vi không xác định ngoài phạm vi dự định của nó.")
...
def HQQ_loader(model_name):
    ...
    logger.info(f"Đang tải mô hình HQQ với phần phụ trợ: \"{shared.args.hqq_backend}\"")
...
def get_max_memory_dict():
    ...
    # for the maximum memory of device :0
        ...
        logger.warning(f"Auto-assiging --gpu-memory {suggestion} để GPU của bạn cố gắng ngăn chặn lỗi hết bộ nhớ. Bạn có thể đặt thủ công các giá trị khác.")
