...
class TemperatureLogitsWarperCustom(LogitsWarper):
    '''
    Một bản sao của Warper ghi lại nhiệt độ Transformers ban đầu.
    '''

    def __init__(self, temperature: float):
        if not isinstance(temperature, float) or not (temperature > 0):
            except_msg = (
                f"`temperature` (={temperature}) phải là một mức thả nổi tích cực, nếu không thì mã thông báo tiếp theo của bạn "
                "điểm số sẽ không hợp lệ."
            )
            if isinstance(temperature, float) and temperature == 0.0:
                except_msg += " Nếu bạn đang tìm kiếm các chiến lược giải mã tham lam, hãy đặt `do_sample=False`."
...
class MinPLogitsWarper(LogitsWarper):
    def __init__(self, min_p: float, filter_value: float = -float("Inf"), min_tokens_to_keep: int = 1):
        if min_p < 0 or min_p > 1.0:
            raise ValueError(f"`min_p` phải là số float >= 0 và <= 1, nhưng {min_p}")
        self.min_p = min_p
...
class TopALogitsWarper(LogitsWarper):
    def __init__(self, top_a: float, filter_value: float = -float("Inf"), min_tokens_to_keep: int = 1):
        ...
        if top_a < 0 or top_a > 1.0:
            raise ValueError(f"`top_a` phải là số float >= 0 và <= 1, nhưng {top_a}")
        self.top_a = top_a
        self.filter_value = filter_value
        self.min_tokens_to_keep = min_tokens_to_keep
...
class MirostatLogitsWarper(LogitsWarper):
    def __init__(self, mirostat_mode: int, mirostat_tau: float, mirostat_eta: float, filter_value: float = -float("Inf"), min_tokens_to_keep: int = 1):
        if mirostat_mode not in [2]:
            raise ValueError(f"`mirostat` phải là số nguyên 2, nhưng {mirostat_mode}")
...
class RepetitionPenaltyLogitsProcessorWithRange(LogitsProcessor):
    '''
    Sao chép từ thư viện máy biến áp
    '''

    def __init__(self, penalty: float, presence_penalty: float, frequency_penalty: float, _range: int):
        if not (penalty > 0):
            raise ValueError(f"`penalty` phải hoàn toàn tích cực, nhưng là {penalty}")
