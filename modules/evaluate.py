...
def calculate_perplexity(models, input_dataset, stride, _max_length):
    ...
    if shared.args.loader == "llama.cpp":
        logger.error("llamacpp_HF là cần thiết để đánh giá độ phức tạp của các mô hình GGUF. Vui lòng tải lại mô hình bằng llamacpp_HF thay vì llama.cpp.")
        raise ValueError

    if shared.args.loader == "ExLlamav2":
        logger.error("Cần có ExLlamav2_HF để đánh giá độ phức tạp của mô hình EXL2. Vui lòng tải lại mô hình bằng ExLlamav2_HF thay vì ExLlamav2.")
        raise ValueError

    if shared.args.loader == "llamacpp_HF" and not shared.args.logits_all:
        logger.error("--logits_all là cần thiết để đánh giá độ phức tạp với các mô hình GGUF. Vui lòng tải lại mô hình với bộ tùy chọn đó/đã chọn.")
        raise ValueError

    if not shared.args.no_use_fast:
        logger.warning("--no_use_fast không được thiết lập. Nếu việc mã hóa tập dữ liệu đầu vào mất nhiều thời gian, hãy thử tải lại mô hình với bộ tùy chọn/đã chọn đó.")

    global past_evaluations
    cumulative_log = ''
    cumulative_log += "Đang tải tập dữ liệu đầu vào...\n\n"
    ...
    for model in models:
        if is_in_past_evaluations(model, input_dataset, stride, _max_length):
            cumulative_log += f"`{model}` đã được thử nghiệm rồi. Bỏ qua.\n\n"
            yield cumulative_log
            continue

        if model != 'current model':
            try:
                yield cumulative_log + f"Đang tải `{model}`...\n\n"
                ...
                cumulative_log += f"Lỗi khi tải `{model}`. Tiếp tục.\n\n"
                yield cumulative_log
                continue

        cumulative_log += f"Đang xử lí `{shared.model_name}`...\n\n"
        yield cumulative_log + "Mã hóa tập dữ liệu đầu vào...\n\n"
        ...
        for begin_loc in tqdm(range(0, seq_len, stride)):
            yield cumulative_log + f"Đánh giá... {100*begin_loc/seq_len:.2f}%"
        ...
        cumulative_log += f"Sự bối rối đối với `{shared.model_name}` là: {float(ppl)}\n\n"
        yield cumulative_log
