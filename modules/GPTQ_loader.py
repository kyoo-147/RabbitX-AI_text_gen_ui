...
def find_quantized_model_file(model_name):
    ...
    if not pt_path:
        for ext in ['.pt', '.safetensors']:
            ...
            if len(found) > 0:
                if len(found) > 1:
                    logger.warning(f'Nhiều hơn một {ext} mô hình đã được tìm thấy. Người cuối cùng sẽ được chọn. Nó có thể sai.')
...
def load_quantized(model_name):
    if shared.args.model_type is None:
        logger.error("Không thể tải mô hình vì không thể suy ra loại của nó từ tên của nó.")
        logger.error("Vui lòng chỉ định loại theo cách thủ công bằng cách sử dụng đối số --model_type.")
        return None
    ...
    if shared.args.pre_layer and model_type == 'llama':
        ...
        if shared.args.pre_layer:
            logger.warning("Bỏ qua --pre_layer vì nó chỉ hoạt động với loại mô hình llama.")

        load_quant = _load_quant
    else:
        logger.error("Đã chỉ định loại mô hình lượng tử hóa trước không xác định. Chỉ hỗ trợ 'llama', 'opt' và 'gptj'")
        exit()
    ...
    if not pt_path:
        ...
        logger.info(f"Tìm thấy mô hình lượng tử hóa sau: {pt_path}")
    ...
    if model_type == 'llama' and shared.args.pre_layer:
        ...
        if shared.args.gpu_memory or torch.cuda.device_count() > 1 or (is_xpu_available() and torch.xpu.device_count() > 1):
            ...
            logger.info("Sử dụng bản đồ thiết bị sau cho mô hình lượng tử hóa:", device_map)
